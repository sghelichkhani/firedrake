...
def _from_hdf5(filename, comm):
    ...
    # Determine the format from an existing h5 group.
    # TODO: Looking for `/topology` group may not be the best.
...
class MeshGeometry(ufl.Mesh, MeshGeometryMixin):
    ...
    def partial_init(self, topology):
        """Partially initialise the mesh.

        :arg topology: The MeshTopology object.
        :kwarg coordinate_element: The UFL element with which the
            coordinate field is represented.
        """
        ...
        def callback(self):
            ...
            coordinates_data = dmcommon.reordered_coords(topology._topology_dm, coordinates_fs.dm.getDefaultSection(),
...
values from f.)"""
    ...
    def view(self, fname, formt):
        vwr = ViewerHDF5()
        vwr.create(fname, mode='w', comm=self._topology_dm.comm)
        vwr.pushFormat(format=formt)
        vwr.pushGroup("mesh0")  # Currently ignored.
        self._topology_dm.view(viewer=vwr)
        vwr.popGroup()
        vwr.popFormat()
        vwr.destroy()
...
def Mesh(meshfile, **kwargs):
    ...
    * HDF5: with extension `.h5`, `.hdf5`
    ...
    geometric_dim = topology._topology_dm.getCoordinateDim()
